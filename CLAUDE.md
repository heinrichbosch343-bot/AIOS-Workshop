...
### /hook [idea, idea-id, or nothing]

**Purpose:** Turn one idea into a hook worth filming, then into the full script for a talking-head short, with the reasoning shown.

**It's a conversation, not a form.** Heinrich types `/hook`, talks through the idea (usually by voice, usually with the actual idea buried in the middle), and the command works it into something filmable across four moves that are never announced on screen: **land the idea** as a claim rather than a topic, **the hook**, **how it moves**, **the script**. He jumps around and the command follows him rather than gating him.

The seven strategy files and the context window load **silently** — no summary printed back — and the repetition guard only speaks when it fires. **Four hooks, not twelve** (twelve on request), each written as speech, each with a reason that names its mechanism and evidence tier, and each with what it costs stated out loud. The command has an opinion about which one to film and says why it beats the second.

Then it writes the spine using the four retention moves, and measures the draft with **`scripts/hook_lab.py`** — the same instrument the swipe corpus uses on other people's videos, pointed at Heinrich's own writing. Deterministic, no API cost. It prints shape, a live benchmark against the corpus, the **Rule of One claim inventory**, and red flags (writing-style bans, em dashes, "not X it's Y", named clients, unverifiable rand figures). **It exits non-zero on job-loss framing or a named client.**

```bash
uv run --no-project --system-certs python scripts/hook_lab.py --script draft.md
uv run --no-project --system-certs python scripts/hook_lab.py --hook "first line here"
```

**The evidence is sorted by tier in `hook-doctrine.md`, and Tier C matters as much as Tier A.** Three papers hold up: Loewenstein 1994 (curiosity is a deprivation response to a gap made *salient*, so the gap must be in something the viewer has a stake in), Berger & Milkman 2012 (high-arousal emotion travels, sadness suppresses), Green & Brock 2000 (story absorption suppresses counter-arguing, which is why the zero-"you" anecdote beats a claim at a sceptical buyer). What does **not** survive: the Zeigarnik effect behind "open loops" (a 2025 meta-analysis of 59 publications found no memory advantage for unfinished tasks), and every "three-second rule" statistic (traces only to content-marketing blogs citing each other). Three seconds stays as a production convention, never as a number said out loud.

Example: `/hook` on its own, then talk. Or `/hook li-027` to start from the board.

### /swipe [url, creator, or nothing]

**Purpose:** Study somebody else's video properly and fold the pattern into the shape layer.

Ingests a public video's caption track with `scripts/swipe_ingest.py` (yt-dlp through uvx, no API key, no cost, media never downloaded), measures how it's delivered, then tears it down against a fixed eleven-field schema so patterns can be counted across the corpus instead of guessed at. The distilled patterns live in `context/linkedin/viral-formulas.md`; the raw material lives in `outputs/content/swipe/` (`swipe.csv` index, `transcripts/`, `captions/`, `teardowns/`).

**The measurement that matters is the breath unit.** YouTube auto-captions carry no punctuation but do carry a timestamp on every word, so the transcript is segmented at pauses instead of at sentences, which is closer to how a talking-head video is actually delivered. **The whole corpus is held at `--gap 0.50`** and every band figure is relative, not absolute (`--gapscan` on a Hormozi video showed a smooth slide from 706 units at 0.20s to 2 at 1.20s, with no natural break). Change the threshold and everything already measured has to be resegmented with `--resegment`.

**Nothing here ever touches LinkedIn programmatically** — no scraping, no cookie, no session, no actor pointed at a profile. A LinkedIn video enters the corpus by Heinrich pasting or dictating what was said (`--paste`). Public YouTube is where the volume comes from.

First corpus captured 2026-08-22: 7 studies, 3.9M views, Alex Hormozi and Dan Martell. The finding that reorganised the rest: **5 of the 7 open with a voice that isn't the creator's**, which is the one thing none of Heinrich's builds currently do.

Example: `/swipe https://www.youtube.com/watch?v=qb10PdKMizM`
...
| `context/linkedin/viral-formulas.md` | **The shape layer.** Five named formulas measured off other creators' videos, what each one transfers, and what doesn't. Load it *after* the topic is chosen, never before |
| `context/linkedin/hook-doctrine.md` | **The evidence layer.** Why the first three seconds hold, sorted by how much survives checking: Tier A peer-reviewed, Tier B traceable practitioner doctrine, **Tier C repeated everywhere and false**. The six hook types and the retention spine. `/hook` reasons from it |
...
8. **Voice notes** — the review-mining tab (rendered by `scripts/crm_voice_tab.py`). One card per firm: the dominant **fixable** problem its own worst Google reviews point at, the WhatsApp deep link, and the script Heinrich reads into a voice note about it. Built on the 18 Aug review pull (`outputs/crm/review-cache-2026-08-18.json`, 6,320 reviews across 317 firms).
   ...
   **Going deep on one prospect:** the corpus is deliberately capped at 20 reviews per firm so all 317 stay comparable. `scripts/scrape_one_firm_reviews.py <place_id> <max_reviews> <out_name>` is the other job — pull the full negative tail for a single firm once they matter. Sorted worst-first, so once the output reaches 4★ you have *every* 1–2★ review that listing has. It writes to its own file (never touching `review-cache-2026-08-18.json`, which this tab and `review_patterns.py` both read) and refuses to overwrite an existing output. ~$0.0006/review. First use: FIXITT, 23 Aug 2026 — 106 reviews, the complete negative tail is 29, and **14 of the 25 written negative reviews are the quote never arriving**. Full analysis in `outputs/research/fixitt-review-analysis-2026-08-23.md`.
...
**Recurring invoicing (cloud, added 2026-08-19):** the Railway backend generates and sends monthly maintenance invoices on its own. `recurring_billing` (Supabase) holds one row per client on a standing monthly deal — amount, billing day, next due date; `services/invoicing.py` renders the Boschly-branded template (`templates/invoice.html`) to a PDF via headless Chromium (Playwright), emails it with the invoice attached, logs it to `invoices`, and rolls `next_invoice_date` forward a month. Runs daily at 07:00 SAST (`services/scheduler.py`), gated by `INVOICE_AUTOSEND_ENABLED` (default off, same pattern as the email drip) — off means every due invoice still gets generated and staged as a Gmail draft with a Telegram ping, so nothing is silently missed while it's off. Invoice numbering (`BOSCHLY-{year}-{seq}`) is computed from the highest existing number in `invoices` at send time. **That only holds if hand-made invoices are recorded in the table too — so whenever Claude writes an invoice by hand, insert the row into `invoices` in the same breath as saving the PDF.** This was not done for `BOSCHLY-2026-004` (Michael Bosch, R4 070.00, 20 Aug 2026, two Claude Code seats): the PDF sits in `outputs/invoices/` while the table stops at 003, so the automated run would have issued a second 004. A number that gets handed out twice is a bookkeeping problem, not a cosmetic one. Cross-check `outputs/invoices/` against the table before trusting the next number. Migration `012_invoicing.sql` seeds the 3 invoices already sent manually (so numbering starts at 004) and activates Chillipepper Projects (R200/month, first automated run 1 Sept 2026); **it has been run** — both tables are live and Chillipepper sits in `recurring_billing` due 2026-09-01. RSST Splicing and Technologies is on an identical R200/month deal per `BOSCHLY-2026-003` but was **not** activated in the seed; add it with an insert into `recurring_billing` when Heinrich wants it automated too. PDF rendering needs Chromium on Railway, added via `apps/boschai-backend/nixpacks.toml` (`playwright install --with-deps chromium` after the normal pip install).

**Check it before the 1st, don't hope:** `GET /invoices/ready` (unguarded, PII-free, same convention as `/quotebot/ready`) answers the questions that decide whether an invoice actually goes out — is the 07:00 job registered on the *live* scheduler and when does it next run, do both tables exist, can the host really render a PDF (it renders one, because a binary being present and a binary being able to start are different questions), can Gmail still authenticate, and is autosend on or is the invoice only being drafted. `GET /invoices/preview?key=…` renders the exact PDF the next run would produce from the live billing row, writing nothing and sending nothing, so September's invoice is readable in August. **The recurring risk is Google, not the code**: the refresh token expires every 7 days while the OAuth consent screen sits in Testing mode, so publish the screen or re-authorise at `/auth/google` before a billing date. Because Telegram is off (`TELEGRAM_ENABLED=0`), invoicing failures are emailed to the connected mailbox instead — `_alert()` never raises, so one client's failed invoice can't crash the loop and skip everybody else's.
...
PDFs are **fpdf2**, not headless Chromium — pure Python, no system packages, no `nixpacks.toml`, ~30ms, and regenerated on demand from the stored row so the document cannot drift from the record. Customer replies land on the same webhook, and **`QUOTE_TECHNICIANS` decides the roles, one way**: a listed number is always the technician, an unlisted number holding a quote from the last 60 days is that quote's customer, anyone else is told to call the office. A yes stamps `accepted_at` and the technician plus Telegram are told — that closes the loop the bad reviews are about. This briefly ran the other way round (role inferred from the data, so one phone could play both parts while recording) and it backfired: once a take had quoted Heinrich's own number, that row sat in the table and every message from his phone was answered as a customer reply, with no way back short of deleting it. The corollary to keep in mind — **a listed number can never receive a quote it can reply to**, so the demo's customer phone must stay off the list. Verify without messaging anyone: `GET /quotebot/ready` (unguarded, PII-free — a `blockers` list saying what to go and do; check this FIRST), `GET /quotebot/channels` (asks Twilio, Google and Paystack whether they still accept our credentials — read-only, cached 60s), `GET /quotebot/status?key=…` (live build string, config, which tables exist, last 25 messages) and `GET /quotebot/selftest?key=…` (13 real technician messages through the real model — it lives on the server because that is where the working Anthropic key is). Nothing sends without a human approving it; only 06x/07x/081-084 are accepted (086/087 look mobile and fail silently, and are refused by name before the send). Gated by `QUOTE_BOT_ENABLED` + `QUOTE_TECHNICIANS`. Demo runs on the **Twilio sandbox** (both phones must send the join code, membership lapses after 72h idle). Production is a one-way door on the number — whatever becomes the API sender leaves the WhatsApp Business app permanently, so never use a published line. Full detail in **`apps/boschai-backend/QUOTE-BOT.md`**; the go-live path (Meta business verification, the WhatsApp sender number, and the one Utility template the first customer message has to become) is in **`apps/boschai-backend/WHATSAPP-TEMPLATE.md`**.
