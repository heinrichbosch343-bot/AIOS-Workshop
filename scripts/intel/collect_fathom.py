...
Usage:
    ...
    python scripts/intel/collect_fathom.py --days 30
"""
    ...
    for base_url in API_ENDPOINTS:
        ...
        while True:
            ...
            try:
                ...
                if status in (404, 405) and base_url != API_ENDPOINTS[-1]:
                    break  # try next endpoint
                print(f"Error: Fathom API returned HTTP {status}.")
                if status == 401:
                    print("  -> API key invalid. Check https://app.fathom.video/settings")
                return []
            except requests.exceptions.ConnectionError:
                if base_url != API_ENDPOINTS[-1]:
                    break  # try next endpoint
                print("Error: Could not connect to Fathom.")
                return []

            # Decode safely — Fathom may return Windows-1252 encoded content
